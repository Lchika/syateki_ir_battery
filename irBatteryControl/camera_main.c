...
void *camera_main(void *arg)
{
  ...
  if(ret < 0){
    printf("ir_battery_control_main: Failed to get NX handle: %d\n", errno);
    return (void *)ERROR;
  }
  ...
  if(ret != 0){
    printf("ERROR: Faild to initialize video: errno = %d\n", errno);
    goto exit_without_cleaning_videodriver;
  }

  // open the device file
  int v_fd = open("/dev/video", 0);
  if(v_fd < 0){
    printf("ERROR: Faild to open video.errno = %d\n", errno);
    ret = ERROR;
  ...
  while(1){
    ...
    nximage_draw((void *)v4l2_buf.m.userptr, VIDEO_HSIZE_QVGA, VIDEO_VSIZE_QVGA);
